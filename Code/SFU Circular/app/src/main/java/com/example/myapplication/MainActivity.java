...
public class MainActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    EditText idTxt,passwordTxt,registerName,registerPass,registerPassConfirm,registerEmail,registerID;
    ...
    public void onClick(View v) {
        ...
        List<UserClass> usersList = dbHelper.getUserInfo();// grabs all existing user info from user database - See DBUserHelper class for more info
        switch(v.getId()){
            case R.id.btnLogin: // actions executed when clicked on the "login button"
                ...
                if(grabLoginId.isEmpty()){ // checks if idTxt is empty
                    idTxt.setError("Field required"); //setError pops a warning msg at the error location
                ...
                for(UserClass x : usersList){ //loops through userList to find if there are such registered user info.
                    if(x.getLoginID().equals(grabLoginId)&&x.getPassword().equals(grabPass)){
                        //login go to next activity
                        finish(); //closes current activity
                        Intent intent = new Intent(MainActivity.this,HomePage.class); // from MainActivity screen to HomePage screen
                        ...
                        Toast.makeText(MainActivity.this,"Logging in...",Toast.LENGTH_SHORT).show(); // pops a helpful msg
                        errorToast=false;
                    }
                }
